...
bayes=function(x,mu1,mu2,d,pi0)pi0*dmvnorm(x,mean=mu1,sigma=diag(d))-(1-pi0)*dmvnorm(x,mean=mu2,sigma =diag(d))
...
for(i in 1:length(d)){
  ...
  print('KDA started..will take time')
...
#  testing plots
#=================
MR_test=matrix(c(MR_Bayes_test,MR_knn_test,MR_svm_1_test,MR_svm_2_test,MR_kde_test,MR_lda_test,MR_qda_test,MR_lm_test,MR_log_test),ncol=length(d),byrow=T)
names=c('Bayes','kNN','SVM_linear','SVM_radial','KDA1','LDA','QDA','lm','logis')
